...
def get_multilid(model, X_test, X_test_noisy, X_test_adv, k=10, batch_size=100, dataset='mnist'):
    ...
    lids_normal, lids_noisy, lids_adv = multiLID(model, X_test, X_test_noisy, X_test_adv, dataset, k, batch_size)
    print("multiLIDs_normal:", lids_normal.shape)
    print("multiLIDs_noisy:", lids_noisy.shape)
    print("multiLIDs_adv:", lids_adv.shape)
...
def main(args):
    ...
    lid_file_X = '{}{}_{}_multiLID_X.npy'.format(multiLID_results_dir, args.dataset, args.attack)
    lid_file_Y = '{}{}_{}_multiLID_Y.npy'.format(multiLID_results_dir, args.dataset, args.attack)
    ...
    print("multiLID: [characteristic shape: ", X.shape, ", label shape: ", Y.shape)
    ...
    import csv
    with open('{}{}_{}.csv'.format(multiLID_results_dir, args.dataset, args.attack), 'w', newline='') as csvfile:
...
if __name__ == "__main__":
    ...
    parser.add_argument('-b', '--batch_size', help="The batch size to use for training.", default=128, type=int)
